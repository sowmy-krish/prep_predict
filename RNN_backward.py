...
for sent in train_Sents:
    ...
    # if v != 0 and sent[v, 0][0].lower() in prepList:
...
# dataSet = {'X_pre':X_pre, 'X_pos':X_pos, 'Y_label':Y_train}
# out = open('dataSet.txt','wb')
...
dense1 = Dense(30, activation='sigmoid')(lstmPos)
drop2 = Dropout(0.6)(dense1)
dense2 = Dense(5, activation='softmax')(drop2)

model = Model(input=S2, output = dense2)
...
model.fit([X_pos_train], Y_train, nb_epoch=numIter, batch_size=200, verbose=1)

print(model.evaluate([X_pos_dev], Y_dev, batch_size=200, verbose=1))
print(model.evaluate([X_pos_test], Y_test, batch_size=200, verbose=1))
