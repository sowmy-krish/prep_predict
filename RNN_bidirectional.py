...
glove = open("/Users/HK/Documents/Courses/2016 Fall/NLP/Final Project/data/glove/glove.6B.300d.txt", "r")
...
S2 = Input(shape=(winSize, embedSize))

lstmPre = LSTM(50, activation = 'tanh', W_regularizer=l2(0.002), return_sequences=False)(S1)
lstmPos = LSTM(50, activation = 'tanh', W_regularizer=l2(0.002), return_sequences=False)(S2)

merged = merge([lstmPre, lstmPos], mode='concat')

drop1 = Dropout(0.6)(merged)
dense1 = Dense(30, activation='sigmoid')(merged)
drop2 = Dropout(0.6)(dense1)
dense2 = Dense(5, activation='softmax')(drop2)

model = Model(input=[S1, S2], output = dense2)
# model = Model(input=S1, output = dense2)
adm = Adamax(lr=0.002, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
model.compile(loss='categorical_crossentropy', optimizer=adm, metrics=['accuracy', 'fbeta_score'])



model.fit([X_pre_train, X_pos_train], Y_train, nb_epoch=numIter, batch_size=200, verbose=1)

print(model.evaluate([X_pre_dev, X_pos_dev], Y_dev, batch_size=200, verbose=1))
print(model.evaluate([X_pre_test, X_pos_test], Y_test, batch_size=200, verbose=1))
